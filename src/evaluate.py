...
def evaluate(env_override=None, model_path_override=None):
    ...
    print(f"  Real audio predicted as Real  (TN) : {tn:,}")
    print(f"  Real audio predicted as Fake  (FP) : {fp:,}")
    print(f"  Fake audio predicted as Real  (FN) : {fn:,}")
    print(f"  Fake audio predicted as Fake  (TP) : {tp:,}")
    print(f"{'='*55}")
    print(f"\n  vs published work:")
